 ...
 */
@WebServlet(description = "Download imagen almacenada en BBDD", urlPatterns = { "/DownloadImagenProfesor" })
public class DownloadImagenProfesor extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("idProfesor") != null) { // comparar con "null" es la forma de comprobar si el par�metro est� en la petici�n web
			String strIdProfesor = request.getParameter("idProfesor");
			try {
				profesor = (Profesor) ProfesorControlador.getControlador().find(Integer.parseInt(strIdProfesor));
				// Una vez que tengo el profesor localizado compruebo si tengo imagen para �l almacenada en BBDD
				
				if (profesor.getImagen() != null && profesor.getImagen().length > 0) {
					...
					response.setContentLength(profesor.getImagen().length);
					...
					InputStream inputStream = new ByteArrayInputStream(profesor.getImagen()); // Convierto el array de bytes en un objeto InputStream
				...
				System.out.println("Profesor no encontrado para el id: " + strIdProfesor);
			}
